...
#
#             # Enhanced course search detection
#             course_keywords = ["course", "courses", "subject", "subjects", "unit", "units", "class", "classes"]
#             level_keywords = ["100", "200", "300", "400", "500", "level", "l"]
#             dept_keywords = ["computer science", "csc", "physics", "phy", "mathematics", "mth", "chemistry", "chm",
#                              "english", "eng", "law", "economics", "eco", "mass comm", "mco"]
#
#             has_course_mention = any(keyword in query for keyword in course_keywords)
#             has_specific_course = bool(re.search(r'\b[A-Z]{2,4}\s*\d{3}\b', query.upper()))
#             has_level_mention = any(keyword in query for keyword in level_keywords)
#             has_dept_mention = any(keyword in query for keyword in dept_keywords)
#
#             if has_course_mention or has_specific_course or has_level_mention or has_dept_mention:
#                 if has_specific_course:
...
#                         # Enhanced course search parameter extraction
#                         search_params = self._extract_course_search_params(query, user_context)
#                         result = self.mcp.execute_tool("search_courses", search_params)
#                         mcp_results.append({"tool": tool, "result": result})
#                         tools_used.append(tool)
#
#                     elif tool == "get_course_info":
